...
W tym rozdziale zajmiemy się podstawowymi pojęciami z zakresu teorii gier wykorzystanymi w niniejszej pracy, jak również
opisem algorytmów kandydujących do wykorzystania w projekcie. Wyjaśnimy też pokrótce ich sposób działania.
...
wartość uzależniona jest od wszystkich strategii granych przez graczy w określonej chwili.
...
liniowego/całkowitoliczbowego lub rekurencyjnego przeszukiwania przestrzeni rozwiązań.

\section{Gra w postaci strategicznej}

\begin{definition}
\textbf{Grą w postaci strategicznej (normalnej)} nazywamy trójkę $(I, S_i, u_i)$, gdzie:
\begin{enumerate}
\item $I = 1, 2, 3, ..., n$ - skończony zbiór $n$ graczy,
\item $S_i$ - przestrzeń strategii czystych zdefiniowana dla każdego gracza $i \in I$,
\item $u_i(s)$ - funkcja wypłaty określająca wypłatę gracza $i \in I$ dla każdego profilu $s \in S$.
\end{enumerate}
\cite[str.~4]{FT-GT}
\end{definition}
...
\begin{definition}
\textbf{Strategią mieszaną} gracza $i$ nazywamy rozkład $\sigma_i$ nad strategiami czystymi $S_i$. Przez $\sigma^{*}$
oznaczymy, profil strategii mieszanych pozostałych graczy. Wówczas funkcja wypłaty ma postać
...
nazywanym \textit{określonym wsparciem} \INENG{particular support}) jest względnie prostym problemem spełnialności.
...
następnie $x_1 + x_2$}
 \FORALL{$S_1^{*} \subseteq S_1$ takich, że $\ABS{S_1^{*}} = x_1$}
  ...
  \IF{$(\not\exists_{s_1 \in S_1}) \textrm{ nie warunkowo zdominowana wzg. } S_2'$}
   \FORALL{$S_2^{*} \subseteq S_2$ takich, że $\ABS{S_2^{*} = x_2}$}
    \IF{$(\not\exists_{s_1 \in S_1}) \textrm{ nie warunkowo zdominowana wzg. } S_2^{*}$}
...
wymienianych są:
\begin{enumerate}
\item algorytm \textbf{podziału symplicjalnego}\INENG{Simplicial Subdivision} TODO \cite{LTH-NE},
...
Wykorzystuje on 2 kolejne procedury - rekurencyjne przeglądanie profili z nawrotami \INENG{Recursive Backgracking} oraz
iteracyjne usuwanie strategii zdominowanych \INENG{Iterated Removal of Strictly Dominated Strategies} (IRSDS).
...
\REPEAT
 ...
 \FORALL{$i \in I$}
  \FORALL{$s_i \in d_i \in D_i$}
   \FORALL{$s_i' \in S_i$}
    \IF{$(\forall_{s_{-1}) \in d_{-i} \in D_{-i}} u_i(s_i,s_{-i}) < u_i(s_i',s_{-1})$}
